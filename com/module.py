...
class SunModule(T5ForConditionalGeneration):
    def __init__(self,config):
        super().__init__(config)

    def init_prompt(self,device):
        emsize = self.shared.weight.size(1)
        print(emsize)
        self.model_device = device
        self.whole_word_embeddings = nn.Embedding(self.config.n_positions,emsize)
        self.whole_sentence_embeddings = nn.Embedding(self.config.n_positions,emsize)
    def input_plus_whole_word(self, input_ids, whole_word_ids, whole_sentence_ids):
        text_emb = self.shared(input_ids)  # (batch_size, src_len, emsize)
        whole_word_emb = self.whole_word_embeddings(whole_word_ids)
        whole_sentence_emb = self.whole_sentence_embeddings(whole_sentence_ids)
        text_emb_plus = text_emb + whole_word_emb + whole_sentence_emb

        return text_emb

    def append_prompt(self, input_ids, whole_word_ids, whole_sentence_ids, attention_mask):
        # prompt
        batch_size = input_ids.size(0)
        # text
        text_emb_plus = self.input_plus_whole_word(input_ids, whole_word_ids, whole_sentence_ids)
        ...
        return text_emb_plus, input_mask
    ...
    def forward(
        ...
        return_dict=None,
        whole_sentence_ids=None
    ):
        if encoder_outputs is None:
            if task_id is None:
                input_emb = self.input_plus_whole_word(input_ids, whole_word_ids,whole_sentence_ids)
            else:
                input_emb, attention_mask = self.append_prompt(task_id, input_ids, whole_word_ids, whole_sentence_ids, attention_mask)# 执 行 这 个

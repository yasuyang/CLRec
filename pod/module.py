...
class Solomon(T5ForConditionalGeneration):
    ...
    def init_prompt(self, task_num, prompts_per_task, device):
        emsize = self.shared.weight.size(1)#这个shared.weight是什么鬼
        self.prompts_per_task = prompts_per_task
        self.model_device = device
        self.prompt_embeddings = nn.Embedding(task_num * prompts_per_task, emsize) # 嵌入字典的大小和每个嵌入向量的大小 9*emsize
        self.whole_word_embeddings = nn.Embedding(self.config.n_positions, emsize)  # sequence length 512*emsiz e
        initrange = 0.1
    ...
    def forward(
        ...
        if encoder_outputs is None:
            if task_id is None:
                ...
                input_emb, attention_mask = self.append_prompt(task_id, input_ids, whole_word_ids, attention_mask)# 执 行 这 个
